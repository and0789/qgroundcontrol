...
Item {
    ...
    /// @return true for an item that finishes the mission -- a landing where it stands, or a return
    /// to launch.
    ///
    /// A wider question than waypointIsLanding, and deliberately a separate one. That test asks
    /// "does this item have an altitude the operator should not be setting", which a return to
    /// launch does not, because it carries no parameters at all. This one asks "does anything after
    /// this item ever run", and the answer is no for both -- which is what the reordering rules
    /// below need.
    function waypointEndsTheMission(index) {
        const command = waypointCommand(index)
        return (command === commandLand) || (command === commandReturnToLaunch)
    }

    /// @return the list position of the plan's takeoff, or -1 when it has none
    function _takeoffPointIndex() {
        const points = missionPoints
        for (var i = 0; i < points.length; i++) {
            if (points[i].isPinned) {
                return i
    ...
    /// @return the list position of the item that finishes the mission, or -1 when there is none
    function _missionEndPointIndex() {
        const points = missionPoints
        for (var i = 0; i < points.length; i++) {
            if (waypointEndsTheMission(points[i].index)) {
                return i
            }
        }
        return -1
    ...
    /// Whether an item may be dragged up or down the plan at all.
    ///
    /// Two items may not. The takeoff has to come first: ArduPilot flies a mission from its first
    /// item, and a plan that begins with anything else does not climb. The item that finishes the
    /// mission has to come last, because nothing after it is ever reached -- moving it up would
    /// silently truncate the plan to whatever sits above it, with the rest still listed and never
    /// flown.
    function waypointIsReorderable(index) {
        const item = _visualItemAt(index)
        return (item !== null) && !_isPinnedItem(item) && !waypointEndsTheMission(index)
    }

    /// How far up and down the list an item may go, as positions in missionPoints.
    ///
    /// Expressed as a range rather than as a yes/no per target so the row's own buttons can grey
    /// themselves out at the ends instead of offering a move that would be refused.
    ///     @return { first, last }, or null when nothing may move
    function reorderRangeForPoints() {
        const points = missionPoints
        if (points.length === 0) {
            return null
        }

        const takeoffAt = _takeoffPointIndex()
        const endAt = _missionEndPointIndex()

        const first = (takeoffAt >= 0) ? (takeoffAt + 1) : 0
        const last  = (endAt >= 0) ? (endAt - 1) : (points.length - 1)
        return (first <= last) ? { first: first, last: last } : null
    }

    /// Moves an item one place up or down the plan.
    ///
    /// Said in list positions rather than visual item indices, because that is what the operator is
    /// looking at: "one row up" is one row up whatever the plan holds that the grid does not draw.
    ///     @param index the item's visual item index
    ///     @param delta -1 for one row up, +1 for one row down
    ///     @return true if it moved
    function moveWaypointByRows(index, delta) {
        if (!canPlaceWaypoints || !missionController || !waypointIsReorderable(index)) {
            return false
        }

        const range = reorderRangeForPoints()
        if (!range) {
            return false
        }
        ...
        if ((target < range.first) || (target > range.last)) {
            return false
        }

        const fromVIIndex = points[at].index
        const toVIIndex   = points[target].index
        missionController.moveVisualItem(fromVIIndex, toVIIndex)

        // A drag of a row is a one-gesture change like any other, so it is takeable back. Recorded
        // as the opposite move rather than as a stored order: the plan has only shifted by one row,
        // and moving it back the other way is exactly that shift undone.
        _recordUndo(qsTr("Undo reorder"), () => moveWaypointByRows(toVIIndex, -delta))
        return true
    }
